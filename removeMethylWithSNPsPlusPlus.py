def processLineMethylShort(line, isTop, CGDist):
	...
	chrom = lineElements[0] #CHANGED from Plus
	position = int(lineElements[1]) #CHANGED from Plus
...
def removeMethylWithSNPsPlus(bismarkMethylationFileName, regionsWithSNPsFileName, CGDist, outputFileName):
	...
	if bismarkMethylationFileNameElements[-1][4:6] == "OB":
		...
		isTop = False
	bismarkMethylationFile = open(bismarkMethylationFileName, 'rb')
...
if __name__=="__main__":
	...
	outputFileName = sys.argv[4] # Should NOT end with .gz

	removeMethylWithSNPsPlus(bismarkMethylationFileName, regionsWithSNPsFileName, CGDist, outputFileName)
