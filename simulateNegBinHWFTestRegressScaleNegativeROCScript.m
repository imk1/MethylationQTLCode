function [numSigPooledNegBinFTestScaleCpGsPPpVal, numSigIndivsGenotypeFTestNegBinScaleCpGsPPpVal] = simulateNegBinHWFTestRegressScaleNegativeROCScript(numAlleles, negBinParamsCpGs, numReadsMeanCpGs, numReadsExpList, numSimulations, MAFList, pVal)
...
for iIndex = 1:length(numReadsExpList)
    ...
    for kIndex = 1:length(MAFList)
        ...
        k = MAFList(kIndex);
        [pValsPooled, pValsIndivsGenotype] = simulateRandomReadsNegBinFTestRegressNegative(numAlleles, negBinParamsCpGs, numReadsMeanCpGs, numReads, numSimulations, k);
