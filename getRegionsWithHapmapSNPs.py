...
def getRegionsWithHapmapSNPs(regionsFileName, SNPsFileName, outputFileName):
	...
	SNPsFile.readline() # REMOVE THE HEADER
	...
	for line in SNPsFile:
		...
		SNPStart = int(lineElements[2])
		SNPEnd = SNPStart # Reference allele is listed first
		while region[0] < lineElements[1]:
		...
		while (region[2] < SNPStart) and (region[0] == lineElements[1]):
		...
		while (region[0] == lineElements[1]) and ((region[1] <= SNPEnd) and (region[2] >= SNPStart)):
...
if __name__=="__main__":
	...
	getRegionsWithHapmapSNPs(regionsFileName, SNPsFileName, outputFileName)