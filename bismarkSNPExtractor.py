...
def processBismarkSamLine(bismarkSamLine):
	...
	return [bismarkSamLineElements[0], bismarkChrom, int(bismarkSamLineElements[3]), bismarkSamLineElements[5], bismarkSamLineElements[9]]
					...
					if len(SNPAlt) - 1 == cl[0]:
						...
						for i in range(cl[0]):
							...
							if bismarkSequence[currentLocation + i] != SNPAlt[i + 1]:
								...
								if (bismarkSequence[currentLocation + i] == "T") and (SNPAlt[i + 1] == "C"):#Y
									# Difference probably due to bisulfite treatment
									continue
								if (bismarkSequence[currentLocation + i] == "A") and (SNPAlt[i + 1] == "G"):#Y
									# Difference probably due to bisulfite treatment
									continue
...
def bismarkSNPExtractor(bismarkSamFileName, SNPVcfStartFileName, outputFileName):
	...
	while bismarkSamLine != "":
		...
		[readName, bismarkChrom, bismarkPosition, bismarkCigar, bismarkSequence] = processBismarkSamLine(bismarkSamLine)
		if bismarkChrom == 0:
			# The read maps to an unknown chromosome, mitochondrial DNA, or a sex chromosome
		...
		for SNPInfo in lastSNPInfo:
			...
			if (bismarkChrom == SNPChrom) and ((bismarkStart <= SNPInfo[1]) and (bismarkEnd >= SNPInfo[1])):#Y
				# The SNP overlaps the current read
				currentSNPInfo.append(SNPInfo)
				if (bismarkEnd >= SNPInfo[1] + len(SNPInfo[2]) - 1) and (bismarkEnd >= SNPInfo[1] + len(SNPInfo[3]) - 1):#Y
					# This SNP, including all possible indels, overlaps the current read
...
if __name__=="__main__":
	...
	bismarkSNPExtractor(bismarkSamFileName, SNPVcfStartFileName, outputFileName)
