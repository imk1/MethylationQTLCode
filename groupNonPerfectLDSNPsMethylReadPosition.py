...
def recordRevisedSNPMethylReadPosition(readNameVec, chromName, SNPLocationVec, SNPVec, methylLocationVec, methylVec, methylReadPosVec, outputFile):
	...
	for i in range(len(readNameVec)):
		...
		if readSNPMethylRecorded == False:#Y
			...
			outputFile.write(readName + "\t" + chromName + "\t" + str(SNPLocation) + "\t" + str(SNPVec[i]) + "\t" + chromName + "\t" + str(methylLocationVec[i]) + "\t" + str(methylVec[i]) + "\t" + str(methylReadPosVec[i]) + "\n") # No longer recording sample information

def groupNonPerfectLDSNPsMethylReadPosition(SNPMethylFileName, LDInfoSNPsFileName, chromName, outputFileName, distanceCutoff, rCutoff):
	...
	for line in SNPMethylFile:
		...
		if (currentMethylLocation != lastMethylLocation) and (lastMethylLocation != 0):#Y
			...
			recordRevisedSNPMethylReadPosition(readNameVec, chromName, SNPLocationVecModified, SNPVecModified, methylLocationVec, methylVec, methylReadPosVec, outputFile)
			readNameVec = []
			SNPLocationVec = []
			SNPVec = []
			methylLocationVec = []
		...
		methylReadPosVec.append(int(lineElements[7]))
	[SNPLocationVecModified, SNPVecModified, SNPPairInfoList, atLDInfoFileEnd] = reviseSNPVecsNonPerfect(SNPLocationVec, SNPVec, SNPPairInfoList, LDInfoSNPsFile, distanceCutoff, atLDInfoFileEnd, rCutoff)
	recordRevisedSNPMethylReadPosition(readNameVec, chromName, SNPLocationVecModified, SNPVecModified, methylLocationVec, methylVec, methylReadPosVec, outputFile)
...
if __name__=="__main__":
	...
	groupNonPerfectLDSNPsMethylReadPosition(SNPMethylFileName, LDInfoSNPsFileName, chromName, outputFileName, distanceCutoff, rCutoff)