function [numSigPooledNegBinScaleCpGsPPpVal, numSigIndivsGenotypeNegBinScaleCpGsPPpVal] = simulateNegBinHWFTestScaleROCScript(numAlleles, numDevFromPerfectCorrList, negBinParamsCpGs, numReadsMeanCpGs, numReadsExpList, numSimulations, MAFList, pVal)
...
for iIndex = 1:length(numReadsExpList)
    ...
    for jIndex = 1:length(numDevFromPerfectCorrList)
        ...
        for kIndex = 1:length(MAFList)
            ...
            k = MAFList(kIndex);
            [pValsPooled, pValsIndivsGenotype] = simulateRandomReadsNegBinFTestWithParamsScaleHWPlusPlus(numAlleles, round(j * k * 10), negBinParamsCpGs, numReadsMeanCpGs, numReads, numSimulations, k);
            numSigPooledNegBinScaleCpGsPPpVal(iIndex, jIndex, round(10*k)) = length(find(pValsPooled < pVal));
            numSigIndivsGenotypeNegBinScaleCpGsPPpVal(iIndex, jIndex, round(10*k)) = length(find(pValsIndivsGenotype < pVal));
        end
    end
end