...
def getLDRelatedSNPInfoRemoveInvalidPairs (genotypeFileName, outputFileName, distanceCutoff):
	...
	# 12.  Probability second SNP is alternate allele given first SNP is alternate allele
	# 13.  Probability second SNP is alternate allele given first SNP is reference allele
	previousSNPDict = {}
		...
		for otherSNPLocation in previousSNPDict.keys():
			...
			for i in range(len(SNPInfo[3])):
				...
				if otherSNPInfo[3][i] == 1:
					...
					# At allele 0 for the first SNP and allele 1 for the second SNP
					xZeroOneCount = xZeroOneCount + 1
			xOneOne = float(xOneOneCount)/float(len(SNPInfo[3]))
			xOneZero = float(xOneZeroCount)/float(len(SNPInfo[3]))
			xZeroOne = float(xZeroOneCount)/float(len(SNPInfo[3]))
			...
			probSeondOneGivenFirstOne = 0
			if pOne > 0:
				# There is at least 1 occurrence of the alternate allele for the first SNP
				probSeondOneGivenFirstOne = xOneOne/pOne # Probability second SNP is alternate allele given first SNP is alternate allele
			probSecondOneGivenFirstZero = 0
			if pOne < 1:
				# There is at least 1 read where the first SNP has the reference allele
				probSecondOneGivenFirstZero = xZeroOne/(1 - pOne) # Probability second SNP is alternate allele given first SNP is reference allele
			LDString = str(corr) + "\t" + str(probFirstOneGivenSecondOne) + "\t" + str(probFirstOneGivenSecondZero) + "\t" + str(probSeondOneGivenFirstOne) + "\t" + str(probSecondOneGivenFirstZero)
...
if __name__=="__main__":
	...
	getLDRelatedSNPInfoRemoveInvalidPairs(genotypeFileName, outputFileName, distanceCutoff)