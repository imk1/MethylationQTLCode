def makeFisherExactSNPsMethylationRandScriptPart(SNPMethylFileListFileName, suffix, minReadsCutoff, MAFCutoff, methylCutoff, scriptFile, codePath, pythonPath):
	# Make a script that will run fisherExactSNPsMethylationRand.py on each file in a list
...
def makeFisherExactSNPsMethylationRandListScript(SNPMethylFileListFileName, suffix, minReadsCutoff, MAFCutoff, methylCutoff, scriptFileName, codePath, pythonPath, listStart, listEnd):
		# Make a script that will run fisherExactSNPsMethylationRand.py on each file in a list listEnd - listStart + 1 times
		scriptFile = open(scriptFileName, 'w+')
		for i in range(listStart, listEnd + 1):
			# Make a part of the script for each random trial
			suffixFull = suffix + str(i)
			
			makeFisherExactSNPsMethylationRandScriptPart(SNPMethylFileListFileName, suffixFull, minReadsCutoff, MAFCutoff, methylCutoff, scriptFile, codePath, pythonPath)
		scriptFile.close()
...
if __name__=="__main__":
	...
	makeFisherExactSNPsMethylationRandListScript(SNPMethylFileListFileName, suffix, minReadsCutoff, MAFCutoff, methylCutoff, scriptFileName, codePath, pythonPath, listStart, listEnd)
