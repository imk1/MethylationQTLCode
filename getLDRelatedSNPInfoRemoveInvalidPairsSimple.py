...
def getLDRelatedSNPInfoRemoveInvalidPairs (genotypeFileName, outputFileName, distanceCutoff):
	...
	for line in genotypeFile:
		...
		if (((SNPInfo[1] == "C") and (SNPInfo[2] == "T")) or ((SNPInfo[1] == "T") and (SNPInfo[2] == "C"))) or (((SNPInfo[1] == "G") and (SNPInfo[2] == "A")) or ((SNPInfo[1] == "A") and (SNPInfo[2] == "G"))): #Y
		...
		for otherSNPLocation in previousSNPDict.keys():
			...
			if math.isnan(corr):
				...
				continue
			secondSNPOneCount = float(sum(SNPInfo[3]))
			xOneOneCount = 0
			xOneZeroCount = 0
			...
			if  secondSNPOneCount > 0: #Y
				# There is at least 1 occurrence of the alternate allele for the second SNP
				probFirstOneGivenSecondOne = float(xOneOneCount)/secondSNPOneCount # Probability first SNP is alternate allele given second SNP is alternate allele
			probFirstOneGivenSecondZero = 0
			if secondSNPOneCount < len(SNPInfo[3]): #Y
				# There is at least 1 read where the second SNP has the reference allele
				probFirstOneGivenSecondZero = float(xOneZeroCount)/(float(len(SNPInfo[3])) - secondSNPOneCount) # Probability first SNP is alternate allele given second SNP is reference allele
