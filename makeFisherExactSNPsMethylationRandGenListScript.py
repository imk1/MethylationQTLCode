def makeFisherExactSNPsMethylationRandGenScriptPart(SNPMethylFileListFileName, suffix, minReadsCutoff, MAFCutoff, methylCutoff, scriptFile, codePath, pythonPath):
	# Make a script that will run fisherExactSNPsMethylationRandGen.py on each file in a list
	...
	for line in SNPMethylFileListFile:
		...
		cmd =pythonPath + "/" + "python " + codePath + "/" + "fisherExactSNPsMethylationRandGen.py"
...
def makeFisherExactSNPsMethylationRandGenListScript(SNPMethylFileListFileName, suffix, minReadsCutoff, MAFCutoff, methylCutoff, scriptFileName, codePath, pythonPath, listStart, listEnd):
		# Make a script that will run fisherExactSNPsMethylationRandGen.py on each file in a list listEnd - listStart + 1 times
		scriptFile = open(scriptFileName, 'w+')
		for i in range(listStart, listEnd + 1):
			# Make a part of the script for each random trial
			suffixFull = suffix + str(i)
			
			makeFisherExactSNPsMethylationRandGenScriptPart(SNPMethylFileListFileName, suffixFull, minReadsCutoff, MAFCutoff, methylCutoff, scriptFile, codePath, pythonPath)
		scriptFile.close()
...
if __name__=="__main__":
	...
	makeFisherExactSNPsMethylationRandGenListScript(SNPMethylFileListFileName, suffix, minReadsCutoff, MAFCutoff, methylCutoff, scriptFileName, codePath, pythonPath, listStart, listEnd)
