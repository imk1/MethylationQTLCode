function [mQTLs, mQTLsRandList] = getmQTLsSNPMethylSpear(locationsBetasIndividualsMethylFileName, genotypesFiltFileName, SNPMethylFileName, numPerms)
    ...
    for j = 1:length(methylLocations)
        ...
        % correlation for each
         ...
         if ~isempty(methylIndex)
            ...
            mQTLs(i, methylIndex) = corr(genotypesFilt.data(i,2:numIndividualsPlusOne)', locationsBetasIndividualsMethyl.data(methylIndex, 2:numIndividualsPlusOne)', 'rows', 'complete', 'type', 'Spearman');
         end
    end
end
...
for k = 1:numPerms
    ...
    for i = 1:size(mQTLs, 1)
        ...
        for j = 1:length(methylIndexesList{i})
            % Iterate through the CpGs for the current SNP and get the
            % correlation for each
            mQTLsRandList{k}(i, methylIndexesList{i}(j)) = corr(genotypesFilt.data(i,2:numIndividualsPlusOne)', locationsBetasIndividualsMethylRand(methylIndexesList{i}(j), :)', 'rows', 'complete', 'type', 'Spearman');
        end
    end
end