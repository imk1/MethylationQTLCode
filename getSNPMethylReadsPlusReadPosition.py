def processLineMethylReadPosition(line, methylLetter, isTop, CGDist):
	...
	if call == methylLetter:#Y
		# The C is methylated
		state = 1
	readPosition = int(lineElements[5].strip())
	return [read, chrom, position, state, readPosition]
...
def getSNPMethylReadsPlusReadPosition(bismarkSNPFileName, bismarkMethylationFileName, methylLetter, CGDist, outputFileName):
	...
	[methylRead, methylChrom, methylPosition, methylState, readPosition] = processLineMethylReadPosition(bismarkMethylationFile.readline(), methylLetter, isTop, CGDist)
	if methylRead == "":
	...
	for line in bismarkSNPFile:
		...
		if SNPRead == lastRead:#Y
			...
			if SNPPosition in SNPPositionsOnRead:#Y
 			...
 			for Ci in CInfo:
				...
				outputFile.write(SNPReadFull + "\t" + SNPChrom + "\t" + str(SNPPosition) + "\t" + str(SNPState) + "\t" + Ci[0] + "\t" + str(Ci[1]) + "\t" + str(Ci[2]) + "\t" + str(Ci[3]) + "\n") #CHANGED
		...
		while methylRead == SNPRead:
			...
			outputFile.write(SNPReadFull + "\t" + SNPChrom + "\t" + str(SNPPosition) + "\t" + str(SNPState) + "\t" + methylChrom + "\t" + str(methylPosition) + "\t" + str(methylState) + "\t" + str(readPosition) + "\n")
			CInfo.append([methylChrom, methylPosition, methylState, readPosition]) #CHANGED
			[methylRead, methylChrom, methylPosition, methylState, readPosition] = processLineMethylReadPosition(bismarkMethylationFile.readline(), methylLetter, isTop, CGDist)
			if methylRead == "":
				# At the end of the methylation file, so stop
				atEnd = True
...
if __name__=="__main__":
	...
	getSNPMethylReadsPlusReadPosition(bismarkSNPFileName, bismarkMethylationFileName, methylLetter, CGDist, outputFileName)
