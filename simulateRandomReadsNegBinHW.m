function [pValsPooled, pValsIndivs, pValsIndivsGenotype, negBinParamsPooled, negBinParamsIndivs] = simulateRandomReadsNegBinHW(numAlleles, numDevFromPerfectCorr, numReads, numSimulations, MAF, numReadsData)
% Simulate random reads and compute the number of mQTLs for pooled data,
% individuals data, and individuals data with associations between genotype
% and average methylation, where numbers of reads are drawn from a negative
% binomial distribution whose parameters have been fit to the data
...
numReadsDataAdjustedPooled = round((numReadsData/mean(numReadsData)) * numReads);
negBinParamsPooled = nbinfit(numReadsDataAdjustedPooled);
numReadsDataAdjustedIndivs = round(numReadsDataAdjustedPooled/numIndivs);
clear numReadsDataAdjustedPooled
negBinParamsIndivs = nbinfit(numReadsDataAdjustedIndivs);
clear numReadsDataAdjustedIndivs
